...
### Functional
- [ ] Add one default single-entry agent that classifies requests, gathers context, creates or updates a spec for non-trivial work, critiques its own plan, revises, and delivers in one conversation.
- [ ] Keep the specialist agents available for advanced or fallback usage, but stop documenting them as the default user path.
- [ ] Encode a mandatory breadth-first scan followed by full-depth analysis of the highest-risk surfaces.
- [ ] Add a reusable adversarial-review skill that challenges assumptions, alternatives, edge cases, regression risk, and quality blind spots.
- [ ] Add a reusable requirements-traceability skill that maps the user request or spec to planned work, verification, and remaining gaps.
- [ ] Add one single-entry workflow prompt that users can paste into the default agent for almost every task.
- [ ] Rewrite the existing greenfield and brownfield prompts so they route through the new single-entry workflow instead of requiring manual agent switching.
- [ ] Add sharper prompt entry points for focused bugfix work and repo-wide launch-readiness audits.
- [ ] Add one researcher fallback agent for external docs, ecosystem references, and launch-readiness benchmarking.
- [ ] Reconcile stale ownership and scoped guidance files outside `.github/` and `docs/` when they would mislead the orchestrator.
- [ ] Update AGENTS and setup documentation so every referenced agent, prompt, and skill exists and matches the implemented workflow.

### Non-functional
- [ ] Performance: the new workflow must not require repeated manual handoffs for normal tasks.
- [ ] Security: the new critique flow must explicitly inspect input validation, secrets hygiene, auth, and query-safety gaps when relevant.
- [ ] Accessibility: UI-oriented prompts and critique paths must explicitly consider accessibility checks.
- [ ] Observability: service or endpoint work must explicitly consider logging, metrics, tracing, and health checks.
...
- [ ] The single-entry orchestrator agent exists and is documented as the default path.
- [ ] The default workflow explicitly does breadth-first discovery before a full-depth pass.
- [ ] The adversarial-review and requirements-traceability skills exist and are referenced by the new default workflow.
- [ ] The specialist agents remain available but are described as fallback/manual modes.
- [ ] The researcher fallback agent exists for external references and benchmark-style work.
- [ ] The greenfield and brownfield prompts no longer require manual agent switching.
- [ ] Focused prompt wrappers exist for bugfix work and launch-readiness audits.
- [ ] `AGENTS.md`, `.github/copilot-instructions.md`, `docs/runbooks/agent-mode.md`, and `docs/copilot-setup.md` all describe the same workflow.
- [ ] Stale repo guidance outside `.github/` and `docs/` that would misroute the workflow has been corrected.
- [ ] All agent, prompt, and skill references in the updated docs resolve to existing files.
- [ ] The new default workflow explicitly addresses planning, critique, verification, observability, security, and testing.
