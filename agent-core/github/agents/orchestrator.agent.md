...
## Phase 1 — Classify
Determine task type: trivial, research-only, review-only, brownfield, greenfield,
implement-from-spec, test-only, docs-only.
...
- framing: real problem vs symptom
- requirements: missing or untestable conditions
- design: simpler alternative, tighter coupling, future constraints
- failure modes: invalid input, empty state, concurrency, partial failure
- quality blind spots: security, performance, testing, docs drift
...
- Compare evidence, not intuition.
